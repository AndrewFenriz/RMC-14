...
public sealed class FiremanCarrySystem : EntitySystem
{
    ...
    private void OnCarriableDragDropDragged(Entity<FiremanCarriableComponent> ent, ref DragDropDraggedEvent args)
    {
        ...
        {
            _popup.PopupClient(Loc.GetString("rmc-fireman-carry-no-skill"), ent, user, PopupType.MediumCaution);
            return;
        }

        if (!carrier.AggressiveGrab)
        {
            _popup.PopupClient(Loc.GetString("rmc-fireman-carry-need-aggressive"), ent, user, PopupType.MediumCaution);
            return;
        }
        ...
        _popup.PopupClient(Loc.GetString("rmc-fireman-carry-start", ("target", target)), ent, user, PopupType.Medium);
    }
