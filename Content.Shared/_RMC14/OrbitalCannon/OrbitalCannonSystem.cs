...
public sealed class OrbitalCannonSystem : EntitySystem
{
    ...
    private void OnWarheadPowerLoaderInteract(Entity<OrbitalCannonWarheadComponent> ent, ref PowerLoaderInteractEvent args)
    {
            ...
            cannon.Status != OrbitalCannonStatus.Unloaded)
        {
            ...
            {
                _popup.PopupClient(Loc.GetString("rmc-ob-tray-loaded"), args.Target, buckled, PopupType.MediumCaution);
            }
        ...
        if (!_container.Insert(args.Used, container))
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("rmc-ob-cannot-insert", ("used", args.Used), ("target", args.Target)), args.Target, buckled, PopupType.MediumCaution);
            }
        }

        _popup.PopupClient(Loc.GetString("rmc-ob-load-success", ("used", args.Used), ("target", args.Target)), args.Target, args.Target, PopupType.Medium);
    ...
    private void OnFuelPowerLoaderInteract(Entity<OrbitalCannonFuelComponent> ent, ref PowerLoaderInteractEvent args)
    {
            ...
            cannon.Status != OrbitalCannonStatus.Unloaded)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("rmc-ob-tray-loaded"), buckled, PopupType.MediumCaution);
            }
            ...
            warheadContainer.ContainedEntities.Count == 0)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("rmc-ob-fuel-no-warhead", ("target", args.Target)), args.Target, buckled, PopupType.MediumCaution);
            }
        ...
        if (fuelContainer.ContainedEntities.Count >= tray.MaxFuel)
        {
            foreach (var buckled in args.Buckled)
            {
                _popup.PopupClient(Loc.GetString("rmc-ob-fuel-full", ("target", args.Target)), args.Target, buckled, PopupType.MediumCaution);
            }
    ...
    public bool Fire(Entity<OrbitalCannonComponent> cannon, Vector2i fireCoordinates, EntityUid user, EntityUid squad)
    {
        ...
        if (cannon.Comp.LinkedTray is not { } trayId || !TryComp(trayId, out OrbitalCannonTrayComponent? tray))
        {
            _popup.PopupCursor(Loc.GetString("rmc-ob-no-tray"), user, PopupType.LargeCaution);
            return false;
        }
        ...
        if (!_container.TryGetContainer(trayId, tray.WarheadContainer, out var warheadContainer) ||
            warheadContainer.ContainedEntities.Count == 0)
        {
            _popup.PopupCursor(Loc.GetString("rmc-ob-no-ammo"), user, PopupType.LargeCaution);
            return false;
        }

        if (!_rmcPlanet.TryPlanetToCoordinates(fireCoordinates, out var planetCoordinates))
        {
            _popup.PopupCursor(Loc.GetString("rmc-ob-out-of-bounds"), user, PopupType.LargeCaution);
            return false;
        }
        ...
        if (!_area.CanOrbitalBombard(_transform.ToCoordinates(planetCoordinates), out var roofed))
        {
            if (roofed)
            {
                _popup.PopupCursor(Loc.GetString("rmc-ob-protected-area"), user, PopupType.LargeCaution);
                return false;
            }

            _popup.PopupCursor(Loc.GetString("rmc-ob-underground"), user, PopupType.LargeCaution);
            return false;
        }

        _popup.PopupCursor(Loc.GetString("rmc-ob-accepted"), user, PopupType.Large);
    ...
    public override void Update(float frameTime)
    {
        ...
        while (firingQuery.MoveNext(out var uid, out var firing, out var cannon))
        {
            ...
            if (!firing.Alerted && time > firing.StartedAt + firing.AlertDelay)
            {
                ...
                var msg = Loc.GetString("rmc-ob-announcement-launch-detected");
                ...
                {
                    var msgArea = Loc.GetString("rmc-ob-announcement-impact-area", 
                        ("warhead", firing.WarheadName), 
                        ("area", areaProto.Name));
                    _rmcChat.ChatMessageToMany(msgArea, msgArea, groundFilter, ChatChannel.Radio);
                }
            }
            ...
            if (!firing.BegunFire && time > firing.StartedAt + firing.BeginFireDelay)
            {
                ...
                var msg = Loc.GetString("rmc-ob-announcement-deck-shudders");
