...
public abstract class SharedCampfireSystem : EntitySystem
{
    ...
    private void OnActivateInWorld(Entity<CampfireComponent> ent, ref ActivateInWorldEvent args)
    {
        ...
        {
            if (_net.IsServer)
                _popup.PopupEntity(Loc.GetString("rmc-campfire-start-extinguishing"), ent, args.User);
        }
    }
    ...
    private void OnExtinguishDoAfter(Entity<CampfireComponent> ent, ref CampfireExtinguishDoAfterEvent args)
    {
        ...
        if (_net.IsServer)
            _popup.PopupEntity(Loc.GetString("rmc-campfire-extinguish-success"), ent, args.User);
    }
    ...
    private void OnInteractUsing(Entity<CampfireComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (TryComp<CampfireFuelComponent>(args.Used, out var fuelComp))
        {
            ...
            {
                if (_net.IsServer)
                    _popup.PopupEntity(Loc.GetString("rmc-campfire-fully-fueled"), ent, args.User);
                return;
            }
            ...
            if (_net.IsServer)
                _popup.PopupEntity(Loc.GetString("rmc-campfire-add-fuel"), ent, args.User);
        ...
        {
            if (_net.IsServer)
                _popup.PopupEntity(Loc.GetString("rmc-campfire-needs-fuel"), ent, args.User);
            return;
        }
